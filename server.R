...
# set up data frame and titles for graph:
    ...
    year1 <- paste0("First Year\nn = ", n_eng_1, "   n = ", n_1) #text string for xlabel including sample size
    
    n_2 <-  sum(with(df, year ==2 & score_total > 1), na.rm = TRUE)     
    year2 <- paste0("Second Year\nn = ", n_eng_2, "   n = ", n_2) #text string for xlabel
    
    n_3 <-  sum(with(df, year ==3 & score_total > 1), na.rm = TRUE)     
    year3 <- paste0("Third Year\nn = ", n_eng_3, "   n = ", n_3) #text string for xlabel
    
    n_4 <-  sum(with(df, year ==4 & score_total > 1), na.rm = TRUE)     
    year4 <- paste0("Fourth Year\nn = ", n_eng_4, "   n = ", n_4) #text string for xlabel
    
    df <- rbind(all_eng, df) #combine selection with all eng to plot comparison
 ...
 ## plot description
    ggplot(
      ...
      scale_fill_manual(
        ...
        labels = c("All Engineering", disc_name)
      )+
